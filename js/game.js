...
var princess = {};
var princessesCaught = localStorage.getItem("princesas");
if (!princessesCaught){
    princessesCaught = 0;
}
var stone = {
};
var numeroPiedras = localStorage.getItem("piedras");
if (!numeroPiedras){
    numeroPiedras = 1;
}
var stonesArray = {
    array: [],
    number: numeroPiedras
}
var monster = {
    speed: 32 + Math.floor(princessesCaught/10)*10
};
...
var update = function (modifier) {
	...
	if (
        ...
        localStorage.setItem("piedras", stonesArray.number);
        if (stonesArray.number > aux){
            monster.speed = 32 + Math.floor(princessesCaught/10)*10;
        }
        stonesArray.array = [];
		reset();
	}
